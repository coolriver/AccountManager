...
TableYearOut::TableYearOut(void)
{
}
...
BOOL TableYearOut::OnInitDialog()
{
	TableFather::OnInitDialog();
	CString title = "";
	title.Format("%d年支出记录",Date.GetYear());
	...
	table_list.InsertColumn(3,"支出源",LVCFMT_CENTER);
	table_list.InsertColumn(4,"支出月份",LVCFMT_CENTER);
	...
	for (i = 0,j = 0;i < 12;i++)
	{
		if (NULL != year_p->month_records[i])
		{
			...
			num.Format("%d",j);
			money.Format("%.1lf",year_p->month_records[i]->get_month_money_out_cash() + year_p->month_records[i]->get_month_money_out_card());
			direct = "全部";
			month.Format("%d",i+1);
...
void TableYearOut::OnNMDblclkList(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void TableYearOut::OnNMClickList(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
	// TODO: 在此添加控件通知处理程序代码
	LPNMITEMACTIVATE temp = (LPNMITEMACTIVATE) pNMHDR;
...
void TableYearOut::OnCbnSelchangeCombox()
{
	// TODO: 在此添加控件通知处理程序代码
    CString str;
  //if the control is the EditBox 
	...
	if ("现金" == str)
	{
		money.Format("%.1lf",year_p->month_records[day-1]->get_month_money_out_cash());
	}
	else if ("银行卡(总)" == str)
	{
		money.Format("%.1lf",year_p->month_records[day-1]->get_month_money_out_card());
	}
	else if ("全部" == str)
	{
		money.Format("%.1lf",year_p->month_records[day-1]->get_month_money_out_cash() + year_p->month_records[day-1]->get_month_money_out_card());
	}
	else
	{
		money.Format("%.1lf",year_p->month_records[day-1]->get_month_money_out_cards(FindCard(str)));
	}
	SetCell(::GetDlgItem (m_hWnd,TABLE_LIST),money,nItem,2);
}


void TableYearOut::OnCbnKillfocusCombox()
{
	// TODO: 在此添加控件通知处理程序代码
	::ShowWindow(::GetDlgItem(m_hWnd,COMBOX),SW_HIDE);
}
