...
#define COMBOX                          1029
TableMonthIn::TableMonthIn(void)
{
}
...
BOOL TableMonthIn::OnInitDialog()
{
	TableFather::OnInitDialog();
	CString title = "";
	title.Format("%d年%d月收入记录",Date.GetYear(),Date.GetMonth());
	...
	table_list.InsertColumn(4,"收入日",LVCFMT_CENTER);
	...
	for (i = 0,j = 0;i < 31;i++)
	{
		if (NULL != month_p->day_records[i])
		{
			...
			num.Format("%d",j);
			money.Format("%.1lf",month_p->day_records[i]->get_day_money_in_cash() + month_p->day_records[i]->get_day_money_in_card());
			direct = "全部";
			day.Format("%d",i+1);
...
void TableMonthIn::OnNMDblclkList(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void TableMonthIn::OnNMClickList(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
	// TODO: 在此添加控件通知处理程序代码
	LPNMITEMACTIVATE temp = (LPNMITEMACTIVATE) pNMHDR;
...
void TableMonthIn::OnCbnKillfocusCombox()
{
	// TODO: 在此添加控件通知处理程序代码
	::ShowWindow(::GetDlgItem(m_hWnd,COMBOX),SW_HIDE);
}