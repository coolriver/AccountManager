...
BOOL TableDayOut::OnInitDialog()
{
	TableFather::OnInitDialog();

	// TODO:  在此添加额外的初始化
	CString title = "";
	title.Format("%d年%d月%d日支出记录",Date.GetYear(),Date.GetMonth(),Date.GetDay());
	...
	table_list.InsertColumn(4,"支出说明",LVCFMT_CENTER);
